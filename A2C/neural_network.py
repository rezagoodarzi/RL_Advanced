...
def sample(input_values):
    noise = tf.random_uniform(tf.shape(input_values))
    return tf.argmax(input_values - tf.log(-tf.log((noise))), 1)


def conv(inputs, nf, ks,strides, gain=1.0):
    return tf.layers.conv2d(inputs, nf, ks, padding='same', strides = (strides,strides), activation=tf.nn.relu,
                            kernel_initializer=tf.orthogonal_initializer(gain=gain))

def dense(inputs, n, act=tf.nn.relu, gain=1.0):
    return tf.layers.dense(inputs, n, activation=act, kernel_initializer=tf.orthogonal_initializer(gain=gain))
